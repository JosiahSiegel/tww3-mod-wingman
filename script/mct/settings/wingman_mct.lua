...
-- =====================================================================
-- AGGRESSIVE DIAGNOSTIC: Force a controlled error so the launcher's
-- outer pcall catches it and logs "Failed to load mod file
-- wingman_mct.lua! Error is ...". This tells us EXACTLY what's wrong
-- (file content, syntax, runtime) instead of the misleading
-- "loaded successfully!" that fires even when the body never executes.
-- This is temporary diagnostic instrumentation; will be removed once
-- the actual cause is found.
if true then
    error("[Wingman DIAG] File body reached line 4. If you see this in the log as 'Failed to load mod file wingman_mct.lua! Error is [Wingman DIAG] File body reached line 4', the file IS being loaded and the body IS executing. The next diagnostic (get_mct check) will tell us why registration fails.")
end
