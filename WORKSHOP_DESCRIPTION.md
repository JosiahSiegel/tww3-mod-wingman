...
Let an AI co-pilot take the stick on your campaign and battles. Set rules, turn caps, victory conditions, faction bans, and periodic breaks. Works with Immortal Empires and most other campaigns.
...
- **Rules & Limits** — turn caps, custom victory conditions (own specific settlements / destroy specific factions), banned-faction watcher.
- **Periodic Breaks** — Wingman hands control back every N turns for review.
...
## ⚠️ Known Limitations (v0.1 alpha)

- Scripted battle AI uses the game's standard AI plans (attack / defend / auto) — doesn't perfectly replicate a player's tactical decisions.
- No custom battle maps or new units.
- Tested primarily on Immortal Empires; Realm of Chaos and other campaigns should work but are not fully verified.

## 🐛 Reporting Issues

If Wingman crashes your campaign, please:
1. Open MCT → Wingman → set `wingman_debug_logging = true`.
2. Reproduce the crash.
3. Attach `script_log_*.txt` from your TWW3 install folder.
4. Post on the Workshop discussion tab.
